    ...
    def draw_humans(npimg, humans, imgcopy=False):
        ...
        for human in humans:
            ...
            for i in range(common.CocoPart.Background.value):
                ...
                #print ("Keys : ",human.body_parts.keys())
                body_part = human.body_parts[i]
                #print("Body Part: ",body_part.x)
            ...
            for pair_order, pair in enumerate(common.CocoPairsRender):
                ...
                distance = math.sqrt(sum([(a - b) ** 2 for a, b in zip( centers[pair[0]],  centers[pair[1]])]))
                row.append(distance)
                cv2.putText(npimg, '%.0f' % (distance), centers[pair[1]], cv2.FONT_HERSHEY_SIMPLEX, 0.9, (0, 0, 255))
                #print("Euclidean distance from {} to {}: {}".format( centers[pair[0]], centers[pair[1]], distance))
                #print("pair 0: ",centers[pair[0]])
                #print("pair 1: ",centers[pair[1]])
                #print(centers[pair[0]][1])
                x1 = centers[pair[0]][0]
                y1 = centers[pair[0]][1]
                x2 = centers[pair[1]][0]
                y2 = centers[pair[1]][1]
                ...
                #cv2.putText(npimg, '%.0f' % (angle), centers[pair[1]], cv2.FONT_HERSHEY_SIMPLEX, 0.3, (0, 0, 255))
                #print("Angle : ", angle)
